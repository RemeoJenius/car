...
//        removeTheSuffix();
    ...
    private static void removeTheSuffix() {

        File file=new File("/Volumes/ziyan/test/car.sql"); //指定文件名及路径
        String filename=file.getAbsolutePath();
        if(filename.indexOf(".")>=0)
        {
            filename = filename.substring(0, filename.lastIndexOf("."));
        }
        file.renameTo(new File(filename+"")); //改名
    }
